...
**但这有个未验证的风险。** 若某个键盘增强工具(如 Karabiner-Elements)抓取物理
键盘后经自身虚拟 HID 设备重发事件,而重发事件带非零 PID,物理键盘会被整体
放行,阻断器失效。DriverKit 设备产生的事件通常仍为 `pid=0`,但**本项目尚未
用真实按键验证过这一点**。

在你自己的机器上用真实按键确认之前,请使用显式白名单(默认)。
...
- **「物理输入被阻断」这一核心行为尚未用真实按键验证过。** 开发期间所有测试
  ...
  (见 FINDINGS.md),但请在你自己的机器上实测后再依赖它。
